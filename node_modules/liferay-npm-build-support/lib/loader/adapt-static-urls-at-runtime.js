...
function default_1(context, options) {
    ...
    const { docroot, include, matchSubstring = false } = util_1.replaceTokens(options);
    ...
    filePosixPaths.forEach((filePosixPath) => {
        ...
        const regexp = new RegExp(`"${matchPrefix}${escape_string_regexp_1.default(filePosixPath)}"`, 'g');
        const matches = regexp.exec(content);
        if (!matches) {
            return;
        }
        log.info('adapt-static-urls', `Adapted ${matches.length} occurrences of URL '${filePosixPath}'`);
        modifiedContent = modifiedContent.replace(regexp, `_ADAPT_RT_.adaptStaticURL("${filePosixPath}")`);
    });
