...
function default_1(context) {
    ...
    if (!matches) {
        log.info('namespace-webpack', `No ocurrences of 'webpackJsonp' found; nothing to be done`);
        return content;
    }
    log.info('namespace-webpack', `Replaced ${matches.length} occurences of 'webpackJsonp'`);
    const hash = crypto_1.default.createHash('MD5');
    hash.update(project_1.default.pkgJson['name']);
    hash.update(project_1.default.pkgJson['version']);
    const uuid = hash
        .digest('base64')
        .replace(/\+/g, '_')
        .replace(/\//g, '_')
