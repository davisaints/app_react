...
function htmlDump(report) {
    ...
    const warnings = htmlIf(!!_warnings.length, () => htmlSection('Warnings', htmlList(..._warnings)));
    ...
    const packageProcesses = htmlSection('Summary of package transformations', htmlTable('Package', 'Version', 'Copy phase', 'Pre-babel phase', 'Babel phase', 'Post-babel phase', Object.keys(_packages)
        ...
        const postKeys = Object.keys(post);
        const babelKeys = Object.keys(babel.files);
        const copyNotice = htmlIf(!!copyKeys.length, () => `${copyKeys.length} plugins applied`);
        const preNotice = htmlIf(!!preKeys.length, () => `${preKeys.length} plugins applied`);
        const babelNotice = htmlIf(!!babelKeys.length, () => `${babelKeys.length} files processed`);
        const postNotice = htmlIf(!!postKeys.length, () => `${postKeys.length} plugins applied`);
        return htmlRow(`
						<td>${pkg.name}</td>
						<td>${pkg.version}</td>
						<td>
    ...
    const packageProcessesBundlerDetails = htmlSection('Details of bundler transformations', ...Object.keys(_packages)
        ...
        const preKeys = Object.keys(pre);
        const postKeys = Object.keys(post);
        return htmlIf(!!copyKeys.length || !!preKeys.length || !!postKeys.length, () => htmlSubsection(`
							<a name="${pkgId}-bundler">
								${pkg.name}@${pkg.version}
							</a>
						`, ...htmlIf(!!copyKeys.length, () => copyKeys
            .sort()
        ...
        ], [copy[pluginName].logger], { source: false }))), ...htmlIf(!!preKeys.length, () => preKeys
            .sort()
        ...
        ], [pre[pluginName].logger], { source: false }))), ...htmlIf(!!postKeys.length, () => postKeys
            .sort()
    ...
    const packageProcessesBabelDetails = htmlSection('Details of Babel transformations', ...Object.keys(_packages)
        ...
        const babelKeys = Object.keys(babel.files);
        return htmlIf(!!babelKeys.length, () => htmlSubsection(`
						<a name="${pkgId}-babel">
							${pkg.name}@${pkg.version}
						</a>
						`, `
